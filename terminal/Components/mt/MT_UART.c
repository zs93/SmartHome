...
void MT_UartInit ()
{
  ...
  uartConfig.callBackFunc         = MT_UartProcessZToolData;//定义了ZTOOL_P1所以使用的是MT_UartProcessZToolData这个函数
...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
  ...
  {
    HalUARTRead (port, &ch, 1);//一个一个地读，读完一个缓冲区就清 1 个了，？为什么这样呢，往下看 

    switch (state)//状态机
    {
      case SOP_STATE:
        if (ch == MT_UART_SOF)//MT_UART_SOF 的值默认是 0xFE,所以数据必须 FE 格式开始发送才能进入下
//一个状态，不然永远在这里转圈
          state = LEN_STATE;
        break;
      ...
      // Allocate memory for the data //分配内存空间
        ...
        {
        // Fill up what we can 
          pMsg->hdr.event = CMD_SERIAL_MSG;//注册事件号CMD_SERIAL_MSG
    ...
    //If number of bytes read is equal to data length, time to move on to FCS 
      ...
      // Make sure it's correct 数据校验 
        if ((MT_UartCalcFCS ((uint8*)&pMsg->msg[0], MT_RPC_FRAME_HDR_SZ + LEN_Token) == FSC_Token))
        {
          osal_msg_send( App_TaskID, (byte *)pMsg );//把数据包发送到 OSAL 层，很很重要 
        }
        else
        {
      // deallocate the msg 
          osal_msg_deallocate ( (uint8 *)pMsg );//清空内存
        }
...
void MT_UartProcessZToolData ( uint8 port, uint8 event ) 
{
  uint8 flag = 0;//flag判断是否收到数据
  uint8 i,len = 0;//len记录数据长度
  uint8 buf[128];//串口buffer最大缓冲默认为128
    (void)event;  // Intentionally unreferenced parameter
   while(Hal_UART_RxBufLen(port))//检测串口数据是否完成
   {
     HalUARTRead(port,&buf[len],1);//把数据接收一个一个地放到buf中
     len++;//记录字符数
     flag = 1;//表示已经接到数据
   }
   if(flag == 1)
   {//分配内存空间，为结构体内容+数据内容+1个记录长度的数据
      ...
      pMsg->hdr.event = CMD_SERIAL_MSG; //事件号还是用原来的CMD_SERIAL_MSG
      pMsg->msg = (uint8*)(pMsg+1); // 把数据定位到结构体数据部分
      pMsg->msg[0] = len;//给上层的数据第一个是长度
      for(i = 0; i < len; i++)
      pMsg->msg[i+1] = buf[i];
...
/**************
由上面可知数据包的格式为 len+data 就是 数据长度+数据
