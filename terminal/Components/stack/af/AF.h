...
#define AF_ACK_REQUEST                     0x10   //要求APS应答，这是应用层的应答，只在直接发送（单播）时使用。
#define AF_DISCV_ROUTE                     0x20   // This option is no longer used, and will be taken out later总要包含这个选项
#define AF_EN_SECURITY                     0x40
#define AF_SKIP_ROUTING                    0x80   //设置这个选项将导致设备跳过路由而直接发送消息。终点设备将不向其父亲发送消息。在直接发送（单播）和广播消息时很好用。
...
typedef struct
{
  ...
  uint8          AppDevVer:4;         //Device Version 0x00 为 Version 1.0
  uint8          Reserved:4;             // AF_V1_SUPPORT uses for AppFlags:4.
  uint8          AppNumInClusters;     //终端支持的输入簇的个数 
...
typedef struct
{
  ...
  uint16  DataLength;              // Number of bytes in TransData数据长度
  uint8  *Data;                    //数据
...
typedef struct
{
  osal_event_hdr_t hdr;     /* OSAL Message header 如何处理消息头*/
  uint16 groupId;           /* Message's group ID - 0 if not set */  //如果没有设置组别的话就是0
  uint16 clusterId;         /* Message's cluster ID*/                // 这就是我设定的任务ID
  afAddrType_t srcAddr;     /* Source Address, if endpoint is STUBAPS_INTER_PAN_EP,
                               it's an InterPAN message源地址,如果STUBAPS_INTER_PAN_EP端点,它是一个InterPAN消息 */
  uint16 macDestAddr;       /* MAC header destination short address */  //发送的短地址
  uint8 endPoint;           /* destination endpoint 目标端点*/ //目标的端点
  uint8 wasBroadcast;       /* TRUE if network destination was a broadcast address如果网络的目的地是一个广播地址 */
  ...
  int8  rssi;               /* The received RF power in units dBm 接收到的功率单位dBm*/
  uint8 SecurityUse;        /* deprecated 弃用*/
  uint32 timestamp;         /* receipt timestamp from MAC 从MAC收到时间戳*/
  uint8 nwkSeqNum;          /* network header frame sequence number网络标题帧序号 */
...
typedef struct
{
  ...
  afNetworkLatencyReq_t latencyReq;//枚举结构 必须用noLatencyReqs 填充
} endPointDesc_t;
