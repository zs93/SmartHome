...
//窗帘 P0.0 输出
#ifdef  DEV_CURTAIN
  #define DEV_CURTAIN_OPEN()       do{P0_0 = 1;}while(0);
  #define DEV_CURTAIN_CLOSE()      do{P0_0 = 0;}while(0);
  #define DEV_CURTAIN_BIT          (P0_0)
#endif
/*****************功能模块定义结束**************************/
  ...
  switch ( pkt->clusterId )
  {
    ...
    ******/
    ...
    case SAMPLEAPP_ADDNET_CLUSTERID://入网处理
          ...
          {
            HalLedSet(LED1,LED_ON);//协调者led1 亮
                           ...
                           AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )
            {
             ...
             //  HalUARTWrite(0,"bind one end",12);
               HalLedSet(LED1,LED_OFF);//协调者led1灯灭
            }
            else
            {
              // Error occurred in request to send.
     ...
     if((pkt->cmd.Data[0]=='r' && pkt->cmd.Data[3]=='a') && pkt->srcAddr.addr.shortAddr == 0x0000)
     {
      ...
      #endif
        HalLedSet(LED1,LED_ON);//终端上亮了led1
     }
     #endif
      break;
